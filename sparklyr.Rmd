...
conf <- spark_config_kubernetes(
  master = "k8s://https://35.238.98.9", # Type kubectl cluster-info to get this url.
  version = "3.2",
  image = "us-central1-docker.pkg.dev/project-lee-1/docker/spark:3.2.1",
  driver = "sparklyr",
...
conf$sparklyr.shell.conf <- c( # The following settings should be here to be used for spark-submit.
  ...
  "spark.kubernetes.namespace=lee", # Use the namespace created by install-gcloud.sh
...
sc <- spark_connect(
  config = conf,
  spark_home = "/opt/spark-3.2.1" # Use the folder where Spark was installed by install-spark.sh
)
```
...
Open a new terminal on the virtual machine and press "shift + ` +  c" to request local forward for 4040.
Type `kubectl port-forward sparklyr 4040:4040` on the virtual machine.
